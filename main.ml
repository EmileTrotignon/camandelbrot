...
let squarred (xr, xi) = ((xr *. xr) -. (xi *. xi), 2. *. xr *. xi)
...
let is_bounded it c =
  ...
  if !n = it then None
  else Some (float_of_int !n +. 1. -. (log (log (norm2 !zn)) /. log 2.))

let pixel_color it c =
  match is_bounded it c with
  | None -> white
  | Some x -> iof (foi black -. (foi white /. ((foi it /. foi it) -. x)))
...
let draw_mandelbrot_set_BW x_def y_def (x_center, y_center) width it =
  ...
  for y = 0 to y_def do
    for x = 0 to x_def do
      ...
      set_color (match !bounded with None -> black | Some _ -> white) ;
      plot x y ;
...
let draw_mandelbrot_set_BW_multicore x_def y_def (x_center, y_center) width it
  ...
  let pixel_color_BW it c =
    match is_bounded it c with None -> white | Some _ -> black
  in
  printf "draw_mandelbrot %d %d (%F, %F) %F %d;;" x_def y_def x_center y_center
    width it ;
  fprintf log "draw_mandelbrot %d %d (%F, %F) %F %d;;" x_def y_def x_center
    y_center width it ;
  print_newline () ;
  let height = ft_height x_def y_def width in
  let x_corner = x_center -. (width /. 2.) in
  ...
  let color_matrix =
    Parmap.array_parmap (Array.map (pixel_color_BW it)) complex_matrix
  in
...
let draw_mandelbrot_set_multicore x_def y_def (x_center, y_center) width it
  ...
  let speed_matrix =
    Parmap.array_parmap (Array.map (is_bounded it)) complex_matrix
  in
  ...
  let pixel_color s' =
    ...
    | None -> white
    | Some s ->
        let coeff = foi ch.(iof (s *. 256.)) /. foi maj in
        let c1 = foi ncolor *. coeff *. coeff in
        let coeff_c1 = c1 -. floor c1 in
        rgb_mean
          color_array.(iof (ceil c1))
          color_array.(iof (floor c1))
          coeff_c1
  in
  let color_matrix =
    Parmap.array_parmap (Array.map pixel_color) speed_matrix
  in
  let img = make_image color_matrix in
  draw_image img 0 0 ; printf "done\n"
...
while !b do
  ...
  if s.keypressed then (
    ...
    if s.key = 'w' then
      ncenter := !ncenter +% (0., ft_height x_def y_def !width /. 2.) ;
    if s.key = 's' then
      ncenter := !ncenter +% (0., -.ft_height x_def y_def !width /. 2.) ;
    if s.key = 'w' then ncenter := !ncenter +% (!width /. 2., 0.) ) ;
  if s.button then (
    ncenter :=
      pixel_to_complex s.mouse_x s.mouse_y x_def y_def !center !width
