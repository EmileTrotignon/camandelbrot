...
let print_rgb c = Printf.printf "{r=%d; g=%d; b=%d}" c.r c.g c.b

let print_hsl c = Printf.printf "{h=%F; s=%F; l=%F}" c.h c.s c.l
...
let rgb_of_int i = {r= i / (256 * 256); g= i / 256 mod 256; b= i mod 256}

let int_of_rgb c = (c.r * 256 * 256) + (c.g * 256) + c.b

let positive_float x y = mod_float (mod_float x y +. y) y

let hsl_of_rgb co =
  let conv_1_255 i = float_of_int (i * 2) /. 255. in
  let r, g, b = (conv_1_255 co.r, conv_1_255 co.g, conv_1_255 co.b) in
  let cl = [r /. 2.; g /. 2.; b /. 2.] in
  let lmax = list_max cl and lmin = list_min cl in
  let l = (lmax +. lmin) /. 2. in
  let s =
    if lmax = lmin then 0.
    else if l < 0.5 then (lmax -. lmin) /. (lmax +. lmin)
    else (lmax -. lmin) /. (2. -. lmax -. lmin)
  in
  let h =
    if co.r = co.g && co.r = co.b then 0.
    else
      let h' =
        ( if co.r >= co.g && co.r >= co.b then g -. b
...
let rgb_of_hsl co =
  let conv_255_1 f = int_of_float (255. *. f) in
  let shove_in_0_1 f =
    if f > 1. then f -. 1. else if f < 0. then f +. 1. else f
  in
  ...
  if co.s = 0. then {r= conv_255_1 co.l; g= conv_255_1 co.l; b= conv_255_1 co.l}
    ...
    let t1 =
      if co.l < 0.5 then co.l *. (1. +. co.s)
      else co.l +. co.s -. (co.l *. co.s)
    in
...
let hsl_of_int c = hsl_of_rgb (rgb_of_int c)
...
let hsl_mean c1 c2 coeff =
  ...
  print_hsl c2hsl;*)
  let rhsl =
    { h= float_mean c1hsl.h c2hsl.h coeff
...
let hsl_mean_reverse c1 c2 coeff =
  ...
  print_hsl c2hsl;*)
  let rhsl =
    { h= mod_float (float_mean (c1hsl.h +. 1.) c2hsl.h coeff) 1.
    ; s= float_mean c1hsl.s c2hsl.s coeff
    ; l= float_mean c1hsl.l c2hsl.l coeff }
  in
  (*print_hsl rhsl;*)
...
let rgb_mean c1 c2 coeff =
  let mean i1 i2 coeff =
    int_of_float (sqrt (float_mean (float_of_int (i1 * i1)) (float_of_int (i2 * i2)) coeff))
  in
  let c1' = rgb_of_int c1 in
  let c2' = rgb_of_int c2 in
  int_of_rgb
    { r= mean c1'.r c2'.r coeff
    ; g= mean c1'.g c2'.g coeff
    ; b= mean c1'.b c2'.b coeff }
